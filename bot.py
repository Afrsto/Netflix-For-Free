...
# ────────────────────────────────────────────────────────────────
# Multiple guild support: collect all GUILD_ID_* environment variables
# ────────────────────────────────────────────────────────────────
ALLOWED_GUILD_IDS = set()

for key, value in os.environ.items():
    if key == "GUILD_ID" or key.startswith("GUILD_ID_"):
        if value and value.isdigit():
            ALLOWED_GUILD_IDS.add(int(value))

if not ALLOWED_GUILD_IDS:
    raise ValueError("❌ No valid GUILD_ID or GUILD_ID_* environment variable found.")
...
# Example URL: https://github.com/Afrsto/bot-users/blob/main/users.txt
# ────────────────────────────────────────────────────────────────
GITHUB_REPO = None
...
async def on_ready() -> None:
    ...
    log.info(f"🏠 Allowed guilds: {', '.join(str(gid) for gid in sorted(ALLOWED_GUILD_IDS))}")
    ...
    for gid in ALLOWED_GUILD_IDS:
        ...
        try:
            ...
            log.info(f"✅ Synced {len(synced)} slash command(s) to guild {gid}")
        except Exception as e:
            log.error(f"❌ Failed to sync commands for guild {gid}: {e}")
